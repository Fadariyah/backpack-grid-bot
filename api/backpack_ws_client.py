...
class BackpackWSClient:
    ...
    def initialize_orderbook(self):
        ...
        try:
            # 使用REST API獲取完整訂單簿
            client = BackpackClient(self.api_key, self.secret_key)
            order_book = client.get_order_book(self.symbol, 100)  # 增加深度
        ...
        try:
            ...
            if self.ws_thread and self.ws_thread.is_alive():
                try:
                    ...
                    logger.debug(f"等待线程结束时出错: {e}")
            self.ws_thread = None
            
            # 重置订阅状态（保留订阅列表以便重连后重新订阅）
            self.bid_price = None
            self.ask_price = None
            self.last_price = None
            self.orderbook = {"bids": [], "asks": []}
